...
class ParserSpec extends AnyFunSuite {
  ...
  test(
    ...
    val result: Either[DonneesIncorectesException, Board] = parser.parseBoard("43")
    assert(result === Left(DonneesIncorectesException("Wrong board parameter")))
  }

  test("parse board should throw incorrect parameter exception when not int") {
    val parser = new Parser("43;1,2,E;DDD;1,3,S;GAGAGA;")

    val result: Either[DonneesIncorectesException, Board] =
  ...
  test("parse board should return board") {
    ...
    val board: Either[DonneesIncorectesException, Board] = parser.parseBoard("5,5")
  ...
  test("parse lawnmowers should return wrong input if size not pair") {
    ...
    val result: Either[DonneesIncorectesException, List[Lawnmower]] =
      parser.parseLawnmowers(board, wrongParams, List())
    assert(result === Left(DonneesIncorectesException("Wrong lawnmowers parameter number")))
  }
  ...
  test(
    ...
    val result: Either[DonneesIncorectesException, List[Lawnmower]] =
      parser.parseLawnmowers(board, wrongParams, List())
    assert(
      result === Left(DonneesIncorectesException("Wrong lawnmowers position / orientation"))
    )
  }
  ...
  test(
    ...
    val result: Either[DonneesIncorectesException, List[Lawnmower]] =
      parser.parseLawnmowers(board, wrongParams, List())
    assert(
      result === Left(DonneesIncorectesException("Wrong action : X"))
    )
  }
  ...
  test(
    ...
    val result: Either[DonneesIncorectesException, List[Lawnmower]] =
      parser.parseLawnmowers(board, wrongParams, List())
    assert(
      result === Left(DonneesIncorectesException("Wrong orientation : 3"))
    )
  }
  ...
  test(
    ...
    val result: Either[DonneesIncorectesException, List[Lawnmower]] =
      parser.parseLawnmowers(board, wrongParams, List())
    assert(
      result === Left(DonneesIncorectesException("Wrong position : zaza"))
    )
  }
  ...
  test(
    ...
    val expectedLawnmowers: Either[DonneesIncorectesException, List[Lawnmower]] =
      Right(List(l1, l2, l3))
    val result: Either[DonneesIncorectesException, List[Lawnmower]] =
  ...
  test(
    ...
    val parser = new Parser("43;1,2,E;DDD;1,3,S;GAGAGA;")
    assert(
      parser.parseString() === Left(DonneesIncorectesException("Wrong board parameter"))
    )
  }
  ...
  test(
    ...
    assert(
      parser.parseString() === Left(DonneesIncorectesException("Wrong orientation : 2"))
    )
  }
